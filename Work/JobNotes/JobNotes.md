...
2. 根据项目需求打对应的包，找到目标包是什么架构

   ```
   vim /home/work/workspace2/git_repo/release_705/Output/bin2.0/log/archive.log //该文件是打包日志记录
   // 之后根据编号搜索，比如：26031157 
   ```

3. **708打包流程**

   系统打包环境：`192.168.0.40` 、ssh进入服务器（系统服务器将会执行打包脚本适配不同系统的环境包）在系统打包环境下 pull仓库，同步代码后进行编译。编译后执行脚本开始打包
...
4. **709**打包流程(与708执行打包脚本参数不同，需要分析main.py)
...
5. **705**打包流程
   ...
   // 7. 下载最新病毒库
   cd /home/work/workspace2/git_repo/release_705/Output/bin2.0/zav/zav_db_download
   /opt/python-3.6.15/bin/python3.6 ./main.py		// 自动下载病毒库到当前路径下的update、zav文件夹下。之后正常打包就是携带最新的病毒库
   ```
...
9. 升级界面：
   ...
   - [ ] 在线升级只能升病毒库，在菜单中、离线升级在工具中

   - [ ] 升级作为一个独立进程，使用QProcess调用进程

     > 看老版本那块的通信，总结、实现

10. - [x] 升级708防护日志中病毒库升级日志丢失

11. - [x] 升级708防护日志、白名单等未同步
...
1. 升级界面：
   ...
   1. 升级的model分为了两个：
      ...
      1. upgradeModel:RJJH使用，当中控向RJJH发送升级请求时，**通过该model启动界面**
      2. upgradeProcessModel:供UpdateUI使用，打开界面后，手动下发升级操作，并负责3个过程callback(state、percent、message)，同步切换界面

   2. 升级的启动进程需要重新设置参数

   3. 问题

      1. rv为失败的情况，无法用之前的字段进行区分

         > 已解决

      2. 离线更新，如何选取离线包？选取了包之后能获取到版本信息吗？/能否区分是进行升级/回退？

         > 不可以，所以现在使用的是之前的

      3. 当前版本号？

         > 病毒库已解决，组件版本号通过跟界面一样的方式获取组件版本号

2. - [x] 病毒处理方式新增“仅上报不处理”，在界面和protobuffer中添加该字段

3. - [x] 实时防护界面轻量模式对应的界面需要补充，增加逻辑

     > 需要增加safed同步到界面的配置中，轻量模式/标准防护；收到后切换界面

4. - [x] 菜单栏样式需要修改，带圆角、项间距中间样式不对

5. - [x] 防护界面病毒木马防护少了文件监控

6. - [x] 自定义规则下方添加UI，统一增加自动放行按钮

7. - [x] 设置中心的下拉菜单，改为新的样式，增加功能，恢复默认设置

8. - [x] 实时防护切换失败后的弹窗文案修改

9. 设置中心缺少的功能：

   1. 常规设置-磁盘监控：新增了隔离区空间清理(之前只有日志占用清理)

   2. 密码保护：目前暂不实现

   3. 查杀设置中增加高级设置：里面新增了四项：扫描PUA、扫描缓存、扫描网络驱动、查杀阻止自动熄屏

   4. U盘防护：新增监控模式

      <img src="source/images/JobNotes/image-20251125101914502.png" alt="image-20251125101914502" style="zoom:50%;" />

   5. 系统防护：新增大开关、新增告警模式

      <img src="source/images/JobNotes/image-20251125102021423.png" alt="image-20251125102021423" style="zoom:50%;" />

10. 软件升级存在的问题：

   11. 在线升级/离线选择安装包：无法先识别版本号，只有升级后才能得到版本号

       2. 基于上述问题，无法实现版本回退这个功能
       3. 样例UI给出的升级失败的提示，与之前现有的错误码对不上。目前采用之前的方式，safed反馈原因

12. - [x] 个别ubuntu右上角菜单打不开，参考705的菜单进行设计展示

13. - [x] 中控下发计划任务不生效

14. - [x] 设置中心，磁盘清理改为之前的按天

15. - [x] 设置中心自定义规则添加到信任与隔离界面

16. - [x] 实时防护设置界面跳转到设置中心

17. - [x] 软件启动、退出上报中控动作

18. - [x] 从实时防护设置界面跳转到设置中心出现崩溃：设置中心为了实现最小化，以堆存储的方式在主界面上，当从实时设置界面跳转的时候又新建了设置界面对象导致崩溃。所以当需要跳转设置中心的时候，请求主界面让其打开显示设置界面

19. - [x] 升级适配工具
      1. 配置文件：
         1. 705、707同种存储方式，只需要将对应的配置，替换到新的配置文件中
         2. 708旧版本是使用sqlite存储配置，正常版本是存储proto在config.ini中
      2. 文件粉碎数据库：707、708版本，数据库第一列将文字时间转为时间戳
      3. 网络白名单数据库：708版本独有的，将网络白名单移动到白名单数据库中

20. - [ ] 设置中心增加功能：

      - [x] 系统防护大开关：只作为界面展示使用，同实时防护设置开关

      - [x] 防护次数改为点击的，跳转到防护日志界面
      ...
      - [x] 查杀设置中增加高级设置：里面新增：扫描PUA

21. - [x] 设置中心收到中控下发的锁不生效的问题

22. - [x] 系统防护设置偶现循环，占用内存过大问题
...
9. - [x] 设置中，威胁防护的监控模式，将推荐设置到轻量模式中

10. 鸿蒙打包工具，调研打包时所需要的目录结构，时间需求

    > [Command Line Tools - 下载中心 - 华为开发者联盟](https://developer.huawei.com/consumer/cn/download/command-line-tools-for-hmos)
    >
    > ophm

11. - [x] 鸿蒙708V11更换授权之后病毒库升级不可使用

12. - [x] 实时防护开启按钮，开启的时候给中控上报动作
...
|       三月任务       | 截至时间 | 完成 |
| :------------------: | :------: | :--: |
| 709bug Windows+Linux |          |      |
|                      |          |      |
|                      |          |      |

1. 中控更换过期授权文件，界面没有同步正确信息，过期时间界面没有展示出来，需要在safed中进行打印

2. windows界面bug：实时防护弹窗UI问题
...
3. 鸿蒙打包工具，调研打包时所需要的目录结构，时间需求

   > [Command Line Tools - 下载中心 - 华为开发者联盟](https://developer.huawei.com/consumer/cn/download/command-line-tools-for-hmos)
   >
   > ophm

4. - [x] 版本配置文件保存失败且查杀时执行升级病毒库，病毒库升级失败

5. - [x] 中控下发的定时任务界面没有合并

6. - [x] 防护组件升级会导致卡在获取升级信息界面

     > 原因：个别系统先收到slotComplete再收到slotState

7. - [x] 商务部项目打包最新病毒库版本

   1. 705分支zav/..zav..download目录下执行main.py脚本，更新最新的病毒库之后打目标两个包提测
   2. 若这两个包第一次升级失败，则问佳伟打包bin包再提测
   3. 若第二次升级失败，则可能是第一步打包出现的问题，看是哪里有问题

8. - [x] windows界面，文件备份和恢复，增加下拉框绘制内容

9. - [x] 收到配置的情况下，只满足709中控，708中控下发的tag字段是不会变的，导致708中控下发的配置收不到

10. - [ ] 705增加心跳：授权信息同步

     > 705增加判断，如果是SDK版本增加心跳
     >
     > 4个小时走一次注册

11. - [x] 备份记录界面优化

12. 709适配SDK版本，即增加功能部分的接口，重点在于接口与safed的通信

    > 1. 复用model层与safed通信
    > 2. 重新写一份model/ipc通信逻辑与接口和safed之间的通信
...
65. Delegate的实际运用，他们三者(mvc)是如何调用的，delegate中实现重写的函数是如何被使用的

87. 对于控件布局，使用拉伸控件的情况下，是否设置最大size即可？
