...
5. bug修改：
   ...
   - [ ] 日志刷新会导致保存日志的天数为0，直接清除日志信息

   - [ ] 查杀几次之后，日志会清除并且点查杀没反应
...
9. - [ ] 在打包脚本中修改UOS出包路径，将com.vsecure.chenxinsd_7.0.0.5-24121008_amd64.deb修改为与其他包名相同的格式。同时安装uos包，看它安装的路径是否正确(/opt/apps/chenxinsd)
...
11. - [ ] safed中的关机操作，使用库函数，不直接使用命令。同时不创建进程，因为有开销

12. - [ ] 卸载时卡住，当时只改了poney右键库，这个方德右键库卸载后刷新也有问题
...
208. “clipp” 是一个 **C++ 的第三方库**，全称是 **"Command Line Interface Parser Plus"**，主要用于**命令行参数解析**。它是一个轻量级、单头文件的库，设计目标是提供简单、强大且富有表达力的命令行解析功能，适用于现代 C++（支持 C++11 及以上版本）。

209. Boost 是 C++ 编程中一个非常著名且广泛使用的**第三方库集合**，全称是 **Boost C++ Libraries**。它由 C++ 社区开发和维护，旨在为 C++ 提供高质量、可移植、功能强大的工具和组件，许多功能甚至被认为是标准库的“试验田”（部分 Boost 库后来被纳入 C++ 标准库，例如 C++11 的 shared_ptr 和正则表达式）

210. kaliLinux历史命令自动补全
...
213. switch 语句中使用多个 case 标签共享同一代码块的写法是完全合法的。这种用法在 C++ 中称为 **fall-through（贯穿）**，只要多个 case 标签紧邻且没有中间插入 break，它们会共享后续的代码块
...
219. FramlessWindow(主界面)和scanModel(模型)：
...
221. 信号槽参数的匹配机制，如果不遵守该机制会导致槽函数无法被信号触发
...
235. 关于架构和系统：架构定义了计算机使用的指令集，是计算机软件和硬件通信的桥梁。**有了架构，才可以在架构上安装操作系统**。操作系统用来管理和分配计算机系统资源的；
