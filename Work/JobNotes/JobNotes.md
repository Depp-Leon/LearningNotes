...
15. - [ ] 隔离区操作和查杀任务互斥，即将隔离区操作也放入任务队列中

16. - [ ] 清除威胁过程中，右键查杀卡死

17. - [ ] 查杀上报进度问题，由于查杀引擎两个线程进行查杀，若两个都查杀大文件，会导致他们很长间隔才调用进度上报。所以开启独立线程进行上报

      > 查看哪块没有修改状态，状态未删干净

18. - [ ] 通过中控下载的安装包，桌面双击安装包安装，授权服务器ip地址未填充

      > 可以单独打包测试，为什么执行到prinst时没有获取到这个ip。正常是在JYNInstall脚本中获取，看是否在prinst中获取到

19. - [ ] 添加、删除信任区数据及删除、恢复隔离区数据动作日志没有上报到中控(黑白隔离区先加)
